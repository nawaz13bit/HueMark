...
{
  // HueMark: the generic pdf.js dist's cross-origin guard exists because a
  // *hosted* viewer page can be embedded/linked by any third-party site and
  // pointed at an arbitrary ?file= URL (SSRF/XSS surface). Our viewer isn't
  // hosted or embeddable that way - it's only reachable via HueMark's own
  // declarativeNetRequest redirect (background.js), so the file origin is
  // always the page the user actually navigated to. Skip the check.
